...
export type ProductsBrandFilter = {
  id: string
  name: string
  slug: string
  count: number
...
interface ProductsBrandSidebarProps {
  brands: ProductsBrandFilter[]
  totalCount: number
  activeBrand?: string | null
}

export function ProductsBrandSidebar({
  brands,
  totalCount,
  activeBrand,
}: ProductsBrandSidebarProps) {
      ...
      <nav aria-label="Filtrar por marca" className="lg:sticky lg:top-32 lg:space-y-10">
        <section className="border-b border-outline-variant pb-6">
          ...
          <ul className="space-y-3">
            <li>
              <Link href="/productos" className={linkClass(!activeBrand)}>
                <span>Todas</span>
